...
__device__ uint scanLSB(const uint val, uint* s_data)
{
    ...
    idx += hipBlockDim_x; // += 128 in this case
                              ...
                              uint4* keysIn,  uint4* valuesIn)
{
    ...
    const uint i = hipThreadIdx_x + (hipBlockIdx_x * hipBlockDim_x);
    const uint tid = hipThreadIdx_x;
    const uint localSize = hipBlockDim_x;
        ...
        uint* blockOffsets, uint startbit, uint numElements, uint totalBlocks)
{
    ...
    uint groupId = hipBlockIdx_x;
    uint localId = hipThreadIdx_x;
    uint groupSize = hipBlockDim_x;

    uint2 radix2;
    radix2 = keys[hipThreadIdx_x + (hipBlockIdx_x * hipBlockDim_x)];
                            ...
                            uint  totalBlocks)
{
    ...
    uint blockId = hipBlockIdx_x;

    uint i = blockId * hipBlockDim_x + hipThreadIdx_x;

    sKeys2[hipThreadIdx_x]   = keys[i];
    sValues2[hipThreadIdx_x] = values[i];

    if(hipThreadIdx_x < 16)
    {
        sOffsets[hipThreadIdx_x]      = offsets[hipThreadIdx_x * totalBlocks +
                                             blockId];
        sBlockOffsets[hipThreadIdx_x] = blockOffsets[blockId * 16 + hipThreadIdx_x];
    }
    __syncthreads();

    uint radix = (sKeys1[hipThreadIdx_x] >> startbit) & 0xF;
    uint globalOffset = sOffsets[radix] + hipThreadIdx_x - sBlockOffsets[radix];

    outKeys[globalOffset]   = sKeys1[hipThreadIdx_x];
    outValues[globalOffset] = sValues1[hipThreadIdx_x];

    radix = (sKeys1[hipThreadIdx_x + GROUP_SIZE] >> startbit) & 0xF;
    globalOffset = sOffsets[radix] + hipThreadIdx_x + GROUP_SIZE -
                   sBlockOffsets[radix];

    outKeys[globalOffset]   = sKeys1[hipThreadIdx_x + GROUP_SIZE];
    outValues[globalOffset] = sValues1[hipThreadIdx_x + GROUP_SIZE];
     ...
     const bool fullBlock, const bool storeSum)
{
    ...
    if (storeSum && tid == hipBlockDim_x-1) {
        g_blockSums[hipBlockIdx_x] = res + threadScanT.w;
    }
...
vectorAddUniform4(uint *d_vector, const uint *d_uniforms, const int n)
{
    ...
    if (hipThreadIdx_x == 0)
    {
        uni[0] = d_uniforms[hipBlockIdx_x];
    }

    unsigned int address = hipThreadIdx_x + (hipBlockIdx_x *
            hipBlockDim_x * 4);
