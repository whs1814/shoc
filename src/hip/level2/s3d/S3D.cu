...
//            hipFuncCachePreferL1));
    ...
    {
        CUDA_SAFE_CALL(hipEventRecord(start, 0));
        hipLaunchKernelGGL(HIP_KERNEL_NAME(ratt_kernel),dim3(blks2),dim3(thrds2),0,s1, gpu_t, gpu_rf, tconv);

        hipLaunchKernelGGL(HIP_KERNEL_NAME(rdsmh_kernel),dim3(blks2),dim3(thrds2),0,s1, gpu_t, gpu_eg, tconv);

        hipLaunchKernelGGL(HIP_KERNEL_NAME(gr_base),dim3(blks2),dim3(thrds2),0,s2, gpu_p, gpu_t, gpu_y,
                                               gpu_c, tconv, pconv);

        hipLaunchKernelGGL(HIP_KERNEL_NAME(ratt2_kernel),dim3(blks2),dim3(thrds2),0,s1,gpu_t, gpu_rf, gpu_rb,
                gpu_eg, tconv);
        hipLaunchKernelGGL(HIP_KERNEL_NAME(ratt3_kernel),dim3(blks2),dim3(thrds2),0,s1, gpu_t, gpu_rf, gpu_rb,
                gpu_eg, tconv);
        hipLaunchKernelGGL(HIP_KERNEL_NAME(ratt4_kernel),dim3(blks2),dim3(thrds2),0,s1, gpu_t, gpu_rf, gpu_rb,
                gpu_eg, tconv);
        hipLaunchKernelGGL(HIP_KERNEL_NAME(ratt5_kernel),dim3(blks2),dim3(thrds2),0,s1, gpu_t, gpu_rf, gpu_rb,
                gpu_eg, tconv);
        hipLaunchKernelGGL(HIP_KERNEL_NAME(ratt6_kernel), dim3(blks2),dim3(thrds2),0,s1, gpu_t, gpu_rf, gpu_rb,
                gpu_eg, tconv);
        hipLaunchKernelGGL(HIP_KERNEL_NAME(ratt7_kernel),dim3(blks2),dim3(thrds2),0,s1,gpu_t, gpu_rf, gpu_rb,
                gpu_eg, tconv);
        hipLaunchKernelGGL(HIP_KERNEL_NAME(ratt8_kernel), dim3(blks2),dim3(thrds2),0,s1,gpu_t, gpu_rf, gpu_rb,
                gpu_eg, tconv);
        hipLaunchKernelGGL(HIP_KERNEL_NAME(ratt9_kernel), dim3(blks2),dim3(thrds2),0,s1, gpu_t, gpu_rf, gpu_rb,
                gpu_eg, tconv);
        hipLaunchKernelGGL(HIP_KERNEL_NAME(ratt10_kernel), dim3(blks2), dim3(thrds2) ,0,s1,gpu_t, gpu_rklow, tconv);

        hipLaunchKernelGGL(HIP_KERNEL_NAME(ratx_kernel), dim3(blks), dim3(thrds), 0,s1, gpu_t, gpu_c, gpu_rf, gpu_rb,
                gpu_rklow, tconv);
        hipLaunchKernelGGL(HIP_KERNEL_NAME(ratxb_kernel), dim3(blks), dim3(thrds), 0,s1, gpu_t, gpu_c, gpu_rf, gpu_rb,
                gpu_rklow, tconv);
        hipLaunchKernelGGL(HIP_KERNEL_NAME(ratx2_kernel), dim3(blks2), dim3(thrds2), 0, s1, gpu_c, gpu_rf, gpu_rb);
        hipLaunchKernelGGL(HIP_KERNEL_NAME(ratx4_kernel), dim3(blks2), dim3(thrds2),0,s1, gpu_c, gpu_rf, gpu_rb);

        hipLaunchKernelGGL(HIP_KERNEL_NAME(qssa_kernel),dim3(blks2),dim3(thrds2),0,s1,gpu_rf, gpu_rb, gpu_a);
        hipLaunchKernelGGL(HIP_KERNEL_NAME(qssab_kernel),dim3(blks), dim3(thrds), 0,s1,gpu_rf, gpu_rb, gpu_a);
        hipLaunchKernelGGL(HIP_KERNEL_NAME(qssa2_kernel),dim3(blks2),dim3(thrds2),0,s1,gpu_rf, gpu_rb, gpu_a);

        hipLaunchKernelGGL(HIP_KERNEL_NAME(rdwdot_kernel), dim3(blks2), dim3(thrds2) ,0,s1, gpu_rf, gpu_rb, gpu_wdot,
                rateconv, gpu_molwt);
        hipLaunchKernelGGL(HIP_KERNEL_NAME(rdwdot2_kernel), dim3(blks2), dim3(thrds2),0,s1, gpu_rf, gpu_rb, gpu_wdot,
                rateconv, gpu_molwt);
        hipLaunchKernelGGL(HIP_KERNEL_NAME(rdwdot3_kernel), dim3(blks2), dim3(thrds2),0,s1, gpu_rf, gpu_rb, gpu_wdot,
                rateconv, gpu_molwt);
        hipLaunchKernelGGL(HIP_KERNEL_NAME(rdwdot6_kernel), dim3(blks2), dim3(thrds2),0,s1, gpu_rf, gpu_rb, gpu_wdot,
                rateconv, gpu_molwt);
        hipLaunchKernelGGL(HIP_KERNEL_NAME(rdwdot7_kernel), dim3(blks2), dim3(thrds2),0,s1, gpu_rf, gpu_rb, gpu_wdot,
                rateconv, gpu_molwt);
        hipLaunchKernelGGL(HIP_KERNEL_NAME(rdwdot8_kernel), dim3(blks2), dim3(thrds2),0,s1, gpu_rf, gpu_rb, gpu_wdot,
                rateconv, gpu_molwt);
        hipLaunchKernelGGL(HIP_KERNEL_NAME(rdwdot9_kernel),dim3(blks2),dim3(thrds2),0,s1, gpu_rf, gpu_rb, gpu_wdot,
                rateconv, gpu_molwt);
        hipLaunchKernelGGL(HIP_KERNEL_NAME(rdwdot10_kernel), dim3(blks2), dim3(thrds2),0,s1, gpu_rf, gpu_rb, gpu_wdot,
                rateconv, gpu_molwt);
        CUDA_SAFE_CALL(hipEventRecord(stop, 0));
        CUDA_SAFE_CALL(hipEventSynchronize(stop));
